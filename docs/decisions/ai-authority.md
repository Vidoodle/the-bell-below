...
The AI DM may interpret player intent; classify an attempt as routine, check, infeasible, or clarify; select a stat and contextual difficulty from the supported scale; cite relevant fact identifiers for internal validation; propose a disclosure-safe rationale plus bounded stakes and effects; choose among authorized NPC behaviors; and narrate committed outcomes. Explicit engine-owned effects supply numeric modifiers outside combat, and no circumstance may affect both the difficulty and a modifier.
